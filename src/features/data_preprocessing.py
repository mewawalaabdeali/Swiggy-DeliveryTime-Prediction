...
def drop_missing_values(data:pd.DataFrame) -> pd.DataFrame:
    logger.info(f"The original data with missing value has {data.shape[0]} rows and {data.shape[1]} columns")
...
if __name__=="__main__":
    ...
    preprocessor = ColumnTransformer(transformers=[
        ...
        ("nomical_encode", OneHotEncoder(drop="first", handle_unknown="ignore", sparse_output=False),nominal_cat_cols),
    ...
    logger.info("Preprocessor save to location")